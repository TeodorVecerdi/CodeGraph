...
namespace CodeGraph.Editor {
    ...
    public class DoWithEachNode : AbstractEndNode {
        public DoWithEachNode() {
            ...
            var loopObjectPort = base.InstantiatePort(Orientation.Horizontal, Direction.Output, Port.Capacity.Multi, typeof(float));
            loopObjectPort.portName = "arrayObject";
            AddOutputPort(loopObjectPort, () => "arrayObject");
        }

        public override string GetCode() {
            var code = $"foreach(var arrayObject in {InputPorts[1].RequestCode()}) {{\n";
