...
namespace CodeGraph.Editor {
    [Node(true, true)]
    [Title("Basic", "Math", "Divide")]
    public class DivideNode : AbstractMiddleNode {
        public DivideNode() {
            Initialize("Divide", DefaultNodePosition);
            ...
            AddInputPort(firstInputPort, () => {
                ...
                if (connections.Count == 0) return $"((object)0)/* WARNING: You probably want connect this node to something. */";
                var output = connections[0].output;
                var node = output.node as AbstractNode;
                if (node == null) return $"((object)0) /* ERROR: Something went wrong and the connected node ended up as null. */";
